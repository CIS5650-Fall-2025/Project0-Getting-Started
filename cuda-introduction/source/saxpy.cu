...
int main(int argc, char *argv[])
{
    ...
    CUDA(cudaMalloc((void **) &d_x, sizeof(float) * size));
    CUDA(cudaMalloc((void **) &d_y, sizeof(float) * size));
    CUDA(cudaMalloc((void **) &d_z, sizeof(float) * size));
    ...
    CUDA(cudaMemcpy(d_x, x, sizeof(float) * size, cudaMemcpyHostToDevice));
    CUDA(cudaMemcpy(d_y, y, sizeof(float) * size, cudaMemcpyHostToDevice));
    ...
    clearHostAndDeviceArray(z, d_z, size); // why need to clear?
    ...
    CUDA(cudaMemcpy(z, d_z, sizeof(float) * size, cudaMemcpyDeviceToHost));
