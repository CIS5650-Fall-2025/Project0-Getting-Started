...
int main(int argc, char *argv[])
{
    ...
    const unsigned sizeX = 2048;
    const unsigned sizeY = 3152;
    ...
    const size_t byteSize = sizeof(float) * (sizeX * sizeY);
    CUDA(cudaMalloc((void**)&d_a, byteSize));
    CUDA(cudaMalloc((void**)&d_b, byteSize));

    // TODO 3: Copy array contents of A from the host (CPU) to the device (GPU)

    CUDA(cudaDeviceSynchronize());
    CUDA(cudaMemcpy(d_a, a, byteSize, cudaMemcpyHostToDevice));
    ...
    std::cout << "***Device To Device Copy***" << std::endl;
    {
        ...
        CUDA(cudaMemcpy(b, d_b, byteSize, cudaMemcpyDeviceToHost));
