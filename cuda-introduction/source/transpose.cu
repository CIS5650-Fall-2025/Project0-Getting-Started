...
__global__ void matrixTransposeNaive(const float* const a, float* const b, const unsigned sizeX, const unsigned sizeY)
{
    ...
    unsigned index_in  = sizeX * j + i;  // Compute input index (i,j) from matrix A
    unsigned index_out = sizeY * i + j;  // Compute output index (j,i) in matrix B = transpose(A)
...
int main(int argc, char *argv[])
{
    ...
    size_t sizeBytes = sizeX * sizeY * sizeof(float);
    CUDA(cudaMalloc(&d_a, sizeBytes));
    CUDA(cudaMalloc(&d_b, sizeBytes));

    // TODO 3: Copy array contents of A from the host (CPU) to the device (GPU)
    CUDA(cudaMemcpy(d_a, a, sizeBytes, cudaMemcpyHostToDevice));
    ...
    std::cout << "***Device To Device Copy***" << std::endl;
    {
        ...
        DIMS dims;
        dims.dimBlock = dim3(11, 13, 1);
        dims.dimGrid = dim3(divup(sizeX, dims.dimBlock.x), divup(sizeY, dims.dimGrid.y), 1);
        ...
        CUDA(cudaMemcpy(b, d_b, sizeBytes, cudaMemcpyDeviceToHost));
