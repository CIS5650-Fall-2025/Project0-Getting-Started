...
int main(int argc, char *argv[])
{
    ...
    CUDA(cudaMalloc((void**)&d_a, sizeof(float) * (sizeX * sizeY)));
    CUDA(cudaMalloc((void**)&d_b, sizeof(float) * (sizeX * sizeY)));

    // TODO 3: Copy array contents of A from the host (CPU) to the device (GPU)
    CUDA(cudaMemcpy(d_a, a, sizeof(float) * (sizeX * sizeY), cudaMemcpyHostToDevice));
    ...
    std::cout << "***Device To Device Copy***" << std::endl;
    {
        ...
        CUDA(cudaMemcpy(b, d_b, sizeof(float) * (sizeX * sizeY), cudaMemcpyDeviceToHost));
