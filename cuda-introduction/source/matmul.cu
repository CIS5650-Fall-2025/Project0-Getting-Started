                                          ...
                                          const unsigned sizeMX, const unsigned sizeNY, const unsigned sizeXY)
{
    ...
    for (int k = 0; k < sizeXY; k++) {
        dot += matrixM[threadIdx.x * sizeXY + k] * matrixN[k * sizeXY + threadIdx.y];
    }
...
int main(int argc, char *argv[])
{
    ...
    // RESULT MATRIX IS sizeMX by size NY

    const unsigned numThreadsX = 16;
    const unsigned numThreadsY = 16;

    const unsigned numBlocksX = divup(sizeMX, numThreadsX);
    const unsigned numBlocksY = divup(sizeNY, numThreadsY);


    DIMS dims;
    dims.dimBlock = dim3(numThreadsX, numThreadsY, 1);
