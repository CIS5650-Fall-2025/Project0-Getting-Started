                                          ...
                                          const unsigned sizeMX, const unsigned sizeNY, const unsigned sizeXY)
{
    ...
    unsigned px = threadIdx.x + blockDim.x * blockIdx.x;
    unsigned py = threadIdx.y + blockDim.y * blockIdx.y;
...
int main(int argc, char *argv[])
{
    ...
    const unsigned sizeMX = 38;
    const unsigned sizeXY = 59;
    const unsigned sizeNY = 83;
    ...
    matrixM = (float*)malloc(sizeMX * sizeXY * sizeof(float));
    matrixN = (float*)malloc(sizeXY * sizeNY * sizeof(float));
    matrixP = (float*)malloc(sizeMX * sizeNY * sizeof(float));
    matrixPGold = (float*)malloc(sizeMX * sizeNY * sizeof(float));
	...
	for (int i = 0; i < sizeMX; i++) {
		for (int j = 0; j < sizeNY; j++) {
            float val = 0.0;
            for (int k = 0; k < sizeXY; k++) {
                val += matrixM[i * sizeXY + k] * matrixN[k * sizeNY + j];
            }
            matrixPGold[i * sizeNY + j] = val;
        }
    }
    // for py -> 0 to sizeNY
    //   for px -> 0 to sizeMX
    ...
    cudaMalloc((void**)&d_matrixM, sizeMX * sizeXY * sizeof(float));
    cudaMalloc((void**)&d_matrixN, sizeXY * sizeNY * sizeof(float));
    cudaMalloc((void**)&d_matrixP, sizeMX * sizeNY * sizeof(float));

    // TODO 5: Copy array contents of M and N from the host (CPU) to the device (GPU)
    cudaMemcpy(d_matrixM, matrixM, sizeMX * sizeXY * sizeof(float), cudaMemcpyHostToDevice);
    cudaMemcpy(d_matrixN, matrixN, sizeXY * sizeNY * sizeof(float), cudaMemcpyHostToDevice);
    ...
    dims.dimGrid  = dim3((sizeMX + 15) / 16, (sizeNY + 15) / 16, 1);
    ...
    cudaMemcpy(matrixP, d_matrixP, sizeMX * sizeNY * sizeof(float), cudaMemcpyDeviceToHost);
