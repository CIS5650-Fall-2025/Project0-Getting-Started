...
int main(int argc, char *argv[])
{
    ...
    const unsigned sizeMX = 156;
    const unsigned sizeXY = 234;
    const unsigned sizeNY = 125;
    ...
    for (unsigned py = 0; py < sizeNY; py++) {
        for (unsigned px = 0; px < sizeMX; px++) {
            float sum = 0;
            for (unsigned k = 0; k < sizeXY; k++) {
                sum += matrixM[px * sizeXY + k] * matrixN[k * sizeNY + py];
            }
            matrixPGold[px * sizeNY + py] = sum;
        }
    }
    ...
    CUDA(cudaMalloc(&d_matrixM, sizeMX * sizeXY * sizeof(float)));
    CUDA(cudaMalloc(&d_matrixN, sizeXY * sizeNY * sizeof(float)));
    CUDA(cudaMalloc(&d_matrixP, sizeMX * sizeNY * sizeof(float)));
