...
void kernelVersionVis(uchar4 *PBOpos, int width, int height, int major,
                      int minor) {
  ...
  // kernel launches
  createVersionVisualization<<<fullBlocksPerGrid, threadsPerBlock>>>(
      PBOpos, width, height, major, minor);
