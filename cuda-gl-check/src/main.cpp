...
bool init(int argc, char **argv) {
  ...
  ss << m_yourName << ": [SM " << m_major << "." << m_minor << "] "
     << deviceProp.name;
  deviceName = ss.str();
...
void initVAO() {
  ...
  glVertexAttribPointer((GLuint)m_positionLocation, 2, GL_FLOAT, GL_FALSE, 0,
                        0);
  glEnableVertexAttribArray(m_positionLocation);

  glBindBuffer(GL_ARRAY_BUFFER, vertexBufferObjID[1]);
  glBufferData(GL_ARRAY_BUFFER, sizeof(texCoords), texCoords, GL_STATIC_DRAW);
  glVertexAttribPointer((GLuint)m_texCoordsLocation, 2, GL_FLOAT, GL_FALSE, 0,
                        0);
